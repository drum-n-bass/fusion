...
let transform = {
  ...
  vue: (svg, componentName, format) => {
    ...
    // these imports are missnamed?
    code = code.replace('createElementVNode', 'createVNode');
    code = code.replace('createElementBlock', 'createBlock');
  ...
  return Promise.all(
    files.map(async (file) => {
        ...
        if (/^([0-9])/.test(componentName)) {
            componentName = componentName.replace(/^([0-9])/, converter.toWords(componentName.match(/^([0-9])/)[0]));
        }

        return {
            svg: await fs.readFile(`./icons/${style}/${file}`, 'utf8'),
            componentName: `${camelcase(componentName.replace(/\.svg$/, ''), {
                pascalCase: true,
            })}Icon`,
        }
    })
...
async function buildIcons(package, style, format) {
  ...
  await Promise.all(
    icons.flatMap(async ({ componentName, svg }) => {
      ...
      // drop a number from the front of the icons name
      let types =
        package === 'react'
...
function main(package) {
  ...
  Promise.all([rimraf(`./${package}/outline/*`), rimraf(`./${package}/solid/*`)])
    .then(() =>
      Promise.all([
        buildIcons(package, 'coins', 'esm'),
        buildIcons(package, 'coins', 'cjs'),
        buildIcons(package, 'interface', 'esm'),
        buildIcons(package, 'interface', 'cjs'),
        buildIcons(package, 'web3', 'esm'),
        buildIcons(package, 'web3', 'cjs'),
        fs.writeFile(`./${package}/coins/package.json`, `{"module": "./esm/index.js"}`, 'utf8'),
        fs.writeFile(`./${package}/interface/package.json`, `{"module": "./esm/index.js"}`, 'utf8'),
        fs.writeFile(`./${package}/web3/package.json`, `{"module": "./esm/index.js"}`, 'utf8'),
      ])
    )
